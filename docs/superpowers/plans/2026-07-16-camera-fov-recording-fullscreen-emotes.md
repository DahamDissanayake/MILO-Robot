...
- **This machine's global Python has `milo-bridge` `pip install -e`'d against the main checkout (`D:\Github\MILO-Robot\bridge`), shared across every git worktree.** Running plain `python -m pytest ...` from inside a worktree silently imports the main checkout's `milo_bridge` package, not the worktree's own edits — tests can pass/fail against the wrong code with no error. Always prefix backend pytest invocations in a worktree with `PYTHONPATH="$(pwd)/bridge"` (repo-root-relative) so the worktree's own copy of the package shadows the global install, e.g. `PYTHONPATH="$(pwd)/bridge" python -m pytest bridge/tests/ -q`. Verify with `PYTHONPATH="$(pwd)/bridge" python -c "import milo_bridge.drivers.camera as c; print(c.__file__)"` if in doubt — it must print a path inside the current worktree.
